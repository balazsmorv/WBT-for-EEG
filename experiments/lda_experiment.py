...
if __name__ == "__main__":
    ...
    mlflow.set_experiment(
        f"LDA_{dataset_name}_{datetime.datetime.now().strftime('%y%m%d-%H%M%S')}"
    )
    try:
            ...
            with mlflow.start_run(run_name=f"Test Subject {subject}"):
                try:
                    ...
                    (
                        ...
                        test_subjects=(
                            int(subject) if dataset_name == "VEPESS" else subject
                        ),
                    ...
                    if dataset_name == "VEPESS":
                        ...
                        param_grid = {
                            ...
                            "lineardiscriminantanalysis__solver": lda_solver,
                            "lineardiscriminantanalysis__shrinkage": lda_shrinkage,
                            "lineardiscriminantanalysis__covariance_estimator": lda_cov_est,
                        }
                        ...
                        if len(np.unique(y_trains)) != 2:
                            param_grid = {
                                ...
                                "onevsrestclassifier__estimator__solver": lda_solver,
                                "onevsrestclassifier__estimator__shrinkage": lda_shrinkage,
                                "onevsrestclassifier__estimator__covariance_estimator": lda_cov_est,
                            }
                        else:
                            param_grid = {
                                "csp__n_components": nfilter,
                ...
                if len(np.unique(y_trains)) != 2:
                    classifier_param_grid = {
                        ...
                        if "lineardiscriminantanalysis" in k
                    }
                else:
                    classifier_param_grid = {
                        k: v
                        for k, v in param_grid.items()
                        if "onevsrestclassifier__estimator" in k
                    }
                ...
                mlflow.log_params(params=args.__dict__)  # set best OT params
                transport = get_transport(ys, args)
                transport.fit(Xs=Xs, ys=ys, Xt=Xt)
